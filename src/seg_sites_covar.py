...
def gen_binned_estimates(rec_dists, s1, s2, **kwargs):
  ...
  for i in range(np.max(bin_idx)):
      ...
      corr_s1_s2_cur = ma.corrcoef(ma.masked_invalid(s1[cur_idx]), ma.masked_invalid(s2[cur_idx]))[0,1]
      se_r_cur = np.sqrt((1. - corr_s1_s2_cur**2)/(n_pairs - 2))
...
class CorrSegSites:
  ...
  def calc_windowed_seg_sites(self, chrom=0,  L=1e3, filt_rec=True, mask=None):
    ...
    windowed_vars, bin_edges = np.histogram(phys_pos[~np.isnan(phys_pos)], bins=bins, weights=weights[~np.isnan(phys_pos)])
    ...
    if mask is not None:
      ...
      df_windows = PyRanges(chromosomes=chrom, starts=bin_edges[:-1], ends=bin_edges[1:])
      df_mask = pyranges.read_bed(mask)
  ...
  def gen_binned_rec_rate(self, chroms=None, **kwargs):
    ...
    rec_rate_mean, rec_rate_se, corr_s1_s2, se_r = gen_binned_estimates(rec_dists, s1, s2, **kwargs)
