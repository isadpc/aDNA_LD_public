...
thousand_genomes_dir = (
    "/home/abiddanda/novembre_lab2/data/external_public/1kg_phase3/haps/"
)
...
rule extract_autosomes:
    ...
    run:
        ...
        shell(
            "bcftools view --threads 4 -s {indiv_str} {input} | bcftools annotate -x INFO,^FORMAT/GT |  bgzip -@4 > {output.vcf}"
        )
...
rule kg_phase3_ancient_merge:
    ...
    input:
        ...
        kg_phase3_vcf=thousand_genomes_dir
...
rule merge_ancient_modern_all:
    ...
    input:
        expand(
            "data/real_data_autosomes/merged_{x}/ancient_{x}_total_merged.chr{CHROM}.vcf.gz",
            CHROM=range(1, 23),
            x=["kgp"],
        ),
