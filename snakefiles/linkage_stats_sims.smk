...
rule est_LDjt_stats_raw:
  ...
  run:
    ...
    pABmod, pABanc,pAmod, pAanc, pBmod, pBanc, Dmod, Danc, gen_dist = TimeStratifiedLDStats.time_strat_hap_freq(haps, rec_pos, times, ta=ta, maf=maf, m=5000, dist_bins="auto", polymorphic_total=polytot, seed=seed)
    # Save the function here ...
    np.savez_compressed(output.ld_stats, pABmod=pABmod, pABanc=pABanc, pAmod=pAmod, pBmod=pBmod, pAanc=pAanc, pBanc=pBanc, Dmod=Dmod, Danc=Danc, rec_dist=gen_dist, maf=maf, ta=ta, scenario=scenario, seed=seed)
...
rule run_est_jtLDstats_raw:
  input:
    expand(config['tmpdir'] +
            ...
            seed=seeds, length=1, Ne=10000, maf=[1,5], polytot=[1])
...
rule collapse_raw_stats:
  ...
  run:
    ...
    for f in tqdm(input.ldfiles):
        ...
        mean_ed0dt, _, _ = binned_statistic(rec_dist, d0dt, np.nanmean, bins=bins)
        std_ed0dt, _, _ = binned_statistic(rec_dist, d0dt, np.nanstd, bins=bins)
        ...
        # Append them all!
        maf_tot.append(maf)
        d0dt_tot.append(mean_ed0dt)
        std_d0dt_tot.append(std_ed0dt)
...
# -------- Verify LD Stats for single recombination rate --------- #   
rule est_sigma_t_verify_bias:
    """ """
    input:
        hap_panel = rules.sim_demography_single_ancients_ldstats.output.hap_panel
    output:
        ld_stats = config['tmpdir'] + 'ld_stats_raw/{scenario}/jointLDstatsverify_mod{mod_n,\d+}_anc{n_anc}_t{ta,\d+}_l{length,\d+}_Ne{Ne,\d+}_{seed,\d+}_maf{maf,\d+}_polytotal{polytot,\d+}.npz'
    run:
        haps_df = np.load(input.hap_panel)
        maf = int(wildcards.maf) / 100.
        haps = haps_df['haps']
        ...
        ta = int(wildcards.ta)
        # Actually compute the joint product of LD
        # NOTE: we have quite a restrictive
        pABmod, pABanc,pAmod, pAanc, pBmod, pBanc, Dmod, Danc, gen_dist = TimeStratifiedLDStats.time_strat_hap_freq(haps, rec_pos, times, ta=ta, maf=maf, m=5000, dist_bins=np.array([1e-4, 5e-3, 1e-2]), polymorphic_total=polytot, seed=seed)
        # Save the function here ...
        np.savez_compressed(output.ld_stats, pABmod=pABmod, pABanc=pABanc, pAmod=pAmod, pBmod=pBmod, pAanc=pAanc, pBanc=pBanc, Dmod=Dmod, Danc=Danc, rec_dist=gen_dist, maf=maf, ta=ta, scenario=scenario, seed=seed)

rule test:
    input:
        expand(config['tmpdir'] + 'ld_stats_raw/{scenario}/jointLDstatsverify_mod{mod_n}_anc{n_anc}_t{ta}_l{length}_Ne{Ne}_{seed}_maf{maf}_polytotal{polytot}.npz', scenario='SerialConstant', ta=[0,100,1000], mod_n=500, n_anc=500, seed=seeds, length=1, Ne=10000, maf=[1,5,10], polytot=[1])        
        
rule collapse_raw_stats_verify_bias:
    input:
        ldfiles = rules.test.input
    output:
