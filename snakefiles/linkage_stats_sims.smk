...
rule sim_demography_single_ancients_ldstats:
    ...
    output:
        ...
        + "hap_copying_raw/hap_panels/{scenario}/hap_panel_{mod_n,\d+}_{n_anc}_{ta,\d+}_{length,\d+}_Ne_{Ne,\d+}_{seed,\d+}.treeseq.gz",
        hap_panel=config["tmpdir"]
        + "hap_copying_raw/hap_panels/{scenario}/hap_panel_{mod_n,\d+}_{n_anc}_{ta,\d+}_{length,\d+}_Ne_{Ne,\d+}_{seed,\d+}.npz",
...
rule est_LDjt_stats_raw:
    ...
    output:
        ld_stats=config["tmpdir"]
        + "ld_stats_raw/{scenario}/jointLDstats_mod{mod_n,\d+}_anc{n_anc}_t{ta,\d+}_l{length,\d+}_Ne{Ne,\d+}_{seed,\d+}_maf{maf,\d+}_polytotal{polytot,\d+}.npz",
    run:
        ...
        (
            ...
            Danc,
            gen_dist,
        ) = TimeStratifiedLDStats.time_strat_hap_freq(
            haps,
            rec_pos,
            times,
            ta=ta,
...
rule collapse_raw_stats:
    ...
    run:
        ...
        for f in tqdm(input.ldfiles):
            ...
            pApB = df["pAmod"] * (1.0 - df["pAanc"]) * df["pBmod"] * (1.0 - df["pBanc"])
            ...
            mean_bins, _, _ = binned_statistic(
                rec_dist, rec_dist, np.nanmean, bins=bins
            )
        ...
        tot_dict = {
            "d0dt": d0dt_tot,
            "std_d0dt": std_d0dt_tot,
            "pApB": pApB_tot,
            "std_pApB": std_pApB_tot,
            "rec_dist": rec_dist_tot,
            "ta": ta_tot,
            "scenario": scenario_tot,
            "maf": maf_tot,
            "seed": seed_tot,
        }
