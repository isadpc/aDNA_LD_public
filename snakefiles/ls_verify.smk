...
def ascertain_variants(hap_panel, pos, maf=0.05, thinning=1):
    ...
    idx = np.where((mean_daf > maf) | (mean_daf < (1.0 - maf)))[0][::thinning]
...
rule infer_scale_real_map:
    ...
    output:
        scale_inf=config["tmpdir"]
        + "ls_verify/results/samp_{n,\d+}.scale_{scale_min,\d+}_{scale_max,\d+}.seed{seed,\d+}.asc{maf,\d+}.rep{rep,\d+}.npz",
    run:
        scale_min, scale_max = int(wildcards.scale_min), int(wildcards.scale_max)
        maf = int(wildcards.maf) / 100.0
        ...
        test_haps = [
            ls_model._sim_haplotype(scale=s, eps=1e-2, seed=int(wildcards.seed))[0]
            for s in scales_true
        ]
        ...
        for i in tqdm(range(scales_true.size)):
            ...
            neg_log_lls = np.array(
                [
                    ls_model._negative_logll(test_haps[i], scale=s, eps=1e-2)
                    for s in scales
                ]
            )
            min_idx = np.argmin(neg_log_lls)
            scales_bracket = (1.0, scales[min_idx] + 100.0)
            # Inferring the marginal
            res = ls_model._infer_scale(
                test_haps[i], eps=1e-2, method="Brent", bracket=scales_bracket
            )
            ...
            res_jt = ls_model._infer_params(
                test_haps[i], x0=[res.x, start_eps], bounds=bounds
            )
...
rule combine_ls_verify:
    input:
        data=lambda wildcards: expand(
            config["tmpdir"]
            + "ls_verify/results/samp_{n}.scale_{scale_min}_{scale_max}.seed{seed}.asc{maf}.rep{rep}.npz",
            n=wildcards.n,
            rep=range(5),
            scale_min=100,
            scale_max=1000,
...
rule infer_scale_global_optimization_test_map:
    output:
        scale_inf=config["tmpdir"]
        + "ls_verify/results/globalopt_samp_{n,\d+}.scale_{scale_min,\d+}_{scale_max,\d+}.seed{seed,\d+}.asc{maf,\d+}.thin{thin,\d+}.rep{rep,\d+}.npz",
    run:
        scale_min, scale_max = int(wildcards.scale_min), int(wildcards.scale_max)
        maf = int(wildcards.maf) / 100.0
        ...
        for i in tqdm(range(scales_true.size)):
            ...
            res_jt = optimize.basinhopping(
                f,
                x0=[500, 5e-2],
                niter=5,
                minimizer_kwargs={
                    "method": "L-BFGS-B",
                    "bounds": [(1.0, 1e4), (1e-5, 0.2)],
                },
            )
            scales_jt_hat[i] = res_jt.x[0]
            eps_jt_hat[i] = res_jt.x[1]
            f1 = lambda x: -ls_model._negative_logll(
                test_haps[i], scale=x, eps=res_jt.x[1]
            )
            f2 = lambda x: -ls_model._negative_logll(
                test_haps[i], scale=res_jt.x[0], eps=x
            )
...
rule combine_ls_verify_global:
    input:
        data=lambda wildcards: expand(
            config["tmpdir"]
            + "ls_verify/results/globalopt_samp_{n}.scale_{scale_min}_{scale_max}.seed{seed}.asc{maf}.thin{thin}.rep{rep}.npz",
            n=wildcards.n,
            rep=[0],
            scale_min=100,
            scale_max=1000,
