...
rule monte_carlo_sasb_sims:
    input:
        expand(
            config["tmpdir"]
            + "hap_copying/hap_panels/{{scenario}}/hap_panel_{{mod_n}}_{{n_anc}}_{{ta}}_{{length}}_Ne_{{Ne}}_{seed}.npz",
            seed=np.arange(1, 21),
        ),
    output:
        corr_SASB=config["tmpdir"]
        + "corr_seg_sites/sims/{scenario}/corr_sA_Sb_{mod_n}_{n_anc}_{ta}_{length}_Ne_{Ne}_{full_seed,\d+}.monte_carlo_L{L,\d+}.N{N,\d+}.npz",
    ...
    run:
        ...
        for i in tqdm(range(20)):
            corr_sim.monte_carlo_corr_SA_SB(
                L=int(wildcards.N), nreps=5000, chrom=i, seed=int(wildcards.full_seed)
            )

        # output slightly less
        rec_rate_mean, rec_rate_se, corr_s1_s2, se_r = corr_sim.gen_binned_rec_rate(
            bins="auto", range=(5e-6, 1e-3)
        )
        np.savez_compressed(
            output.corr_SASB,
...
rule monte_carlo_sasb_sims_v2:
    ...
    output:
        corr_SASB=config["tmpdir"]
        + "corr_seg_sites/sims/{scenario}/corr_sA_Sb_{mod_n}_{n_anc}_{ta}_{length}_Ne_{Ne}_{full_seed,\d+}.monte_carlo_L{L,\d+}.N{N,\d+}.v2.npz",
    wildcard_constraints:
        scenario="(SerialConstant|TennessenEuropean)",
        mod_n="1",
        n_anc="1",
    run:
        ...
        for d in np.logspace(1, 4, 100):
            for i in tqdm(range(20)):
                corr_sim.monte_carlo_corr_SA_SB_v2(
                    chrom=i,
                    dist=d,
                    nreps=5000,
                    L=int(wildcards.L),
                    seed=int(wildcards.full_seed),
                )
        # output slightly less
        rec_rate_mean, rec_rate_se, corr_s1_s2, se_r = corr_sim.gen_binned_rec_rate(
            bins=np.logspace(-7, -3, 25)
        )
        np.savez_compressed(
            output.corr_SASB,
            scenario=wildcards.scenario,
...
rule estimate_monte_carlo_sA_sB_sims:
    input:
        expand(
            config["tmpdir"]
            + "corr_seg_sites/sims/{scenario}/corr_sA_Sb_1_1_{ta}_20_Ne_10000_{full_seed}.monte_carlo_L{L}.N{N}.npz",
            scenario=["SerialConstant", "TennessenEuropean"],
            ta=[0, 1000, 10000],
            L=1000,
            N=200,
        ...
        expand(
            config["tmpdir"]
            + "corr_seg_sites/sims/{scenario}/corr_sA_Sb_1_1_{ta}_20_Ne_6958_{full_seed}.monte_carlo_L{L}.N{N}.npz",
...
rule estimate_monte_carlo_sA_sB_sims_v2:
    input:
        expand(
            config["tmpdir"]
            + "corr_seg_sites/sims/{scenario}/corr_sA_Sb_1_1_{ta}_20_Ne_10000_{full_seed}.monte_carlo_L{L}.N{N}.v2.npz",
            scenario=["SerialConstant", "TennessenEuropean"],
            ta=[0, 1000, 10000],
            L=[1000],
            N=200,
...
rule gen_seg_sites_table_haploid_modern_test:
    ...
    output:
        tmp_vcf=temp(
            config["tmpdir"]
            + "corr_seg_sites/real_data/anc_{ANC}_mod_{MOD}/interpolated_{proj}_{recmap}_chr{CHROM}.haploid_modern_{hap,\d+}.vcf.gz"
        ),
        tmp_vcf_idx=temp(
            config["tmpdir"]
            + "corr_seg_sites/real_data/anc_{ANC}_mod_{MOD}/interpolated_{proj}_{recmap}_chr{CHROM}.haploid_modern_{hap,\d+}.vcf.gz.tbi"
        ),
        pos_ac=config["tmpdir"]
        + "corr_seg_sites/real_data/anc_{ANC}_mod_{MOD}/interpolated_{proj}_{recmap}_chr{CHROM}.haploid_modern_{hap,\d+}.test.npz",
    wildcard_constraints:
    ...
    run:
        ...
        shell(
            "bcftools view -s {indiv_ids} --threads 4 {input.vcf} | bcftools view -v snps -c 1:minor --threads 4 | bgzip -@4 > {output.tmp_vcf}"
        )
        ...
        rec_df = pd.read_csv(
            input.rec_df, sep="\s+", low_memory=True, dtype=rec_map_types
        )
        ...
        np.savez_compressed(
            ...
            pos=pos,
            gt_anc=gt_anc,
            gt_mod_hap=gt_mod_hap,
            rec_pos=interp_rec_pos,
        )
...
rule haploid_modern_test:
    ...
    input:
        expand(
            config["tmpdir"]
            + "corr_seg_sites/real_data/anc_{ANC}_mod_{MOD}/interpolated_{proj}_{recmap}_chr{CHROM}.haploid_modern_{hap}.test.npz",
            ANC=["LBK", "UstIshim"],
            MOD=["SS6004468", "SS6004474", "8.4_BGI_WG", "Yamnaya"],
            recmap="deCODE",
            proj="kgp",
            hap=[1],
            CHROM=range(1, 23),
...
rule diploid_modern_test:
    ...
    input:
        expand(
            config["tmpdir"]
            + "corr_seg_sites/real_data/anc_{ANC}_mod_{MOD}/interpolated_{proj}_{recmap}_chr{CHROM}.diploid.test.npz",
            CHROM=range(1, 23),
            ANC=["LBK", "UstIshim"],
            MOD=["SS6004468", "SS6004474", ""],
            recmap="deCODE",
            proj="kgp",
        ),
...
CENTROMERE_AND_MAPPABILITY = (
    "data/raw_data/genome_masks/exclusion_mask/hs37d5_autosomes.mask.exclude.filt.bed"
)

MASK_DICT = {
    "none": None,
    "pilot": PILOT_MASK,
    "centromere": CENTROMERE_MASK,
    "centromere_mappability": CENTROMERE_AND_MAPPABILITY,
}
...
rule monte_carlo_sA_sB_est_haploid_autosomes:
    ...
    output:
        corr_SASB=config["tmpdir"]
        + "corr_seg_sites/monte_carlo_results/anc_{ANC}_mod_{MOD}/autosomes.paired_seg_sites.{proj}.{recmap}.{mask}.hap{hap,\d+}.seed{seed,\d+}.monte_carlo_L{L,\d+}.N{N,\d+}.npz",
    wildcard_constraints:
        recmap="(COMBINED_LD|deCODE)",
...
pop_1kg_df = pd.read_table(
    "data/raw_data/1kg_panel/integrated_call_samples_v3.20130502.ALL.panel"
)
...
rule sim_mod_haploid_anc_diploid:
    output:
        treeseq=config["tmpdir"]
        + "hap_copying/test_corr_hapdip_hap_panels/{scenario}/hap_panel_{mod_n,\d+}_{n_anc}_{ta,\d+}_{length,\d+}_Ne_{Ne,\d+}_{seed,\d+}.treeseq.gz",
        hap_panel=config["tmpdir"]
        + "hap_copying/test_corr_hapdip_hap_panels/{scenario}/hap_panel_{mod_n,\d+}_{n_anc}_{ta,\d+}_{length,\d+}_Ne_{Ne,\d+}_{seed,\d+}.npz",
    wildcard_constraints:
        scenario="(SerialConstant)",
        mod_n="2",
        n_anc="2",
...
rule conv_hap_diploid:
    input:
        hap_panel=rules.sim_mod_haploid_anc_diploid.output.hap_panel,
    output:
        pos_ac=config["tmpdir"]
        + "hap_copying/test_corr_hapdip_hap_panels/{scenario}/hap_panel_{mod_n,\d+}_{n_anc}_{ta,\d+}_{length,\d+}_Ne_{Ne,\d+}_{seed,\d+}.pos_ac.npz",
    run:
        hap_panel = np.load(input.hap_panel)
        pos = hap_panel["phys_pos"]
        rec_pos = hap_panel["rec_pos"]
        assert pos.size == rec_pos.size
        chrom = np.repeat(int(wildcards.seed), pos.size)
        haps = hap_panel["haps"]
        ta = hap_panel["ta"]
        assert ta.size == 4
        mod_idx = np.where(ta == 0)[0]
        gt_anc = haps[~mod_idx, :].sum(axis=0)
        gt_mod_hap = haps[mod_idx, :][0, :]
        assert gt_anc.ndim == 1
        assert gt_mod_hap.ndim == 1
        np.savez_compressed(
            output.pos_ac,
            chrom=chrom,
            pos=pos,
...
rule monte_carlo_sA_sB_est_haploid_chromosomes_test:
    """
    Estimate of the Monte-Carlo correlation in sA  vs. sB for all chromosomes
    """
    input:
        seg_sites_haploid=expand(
            config["tmpdir"]
            + "hap_copying/test_corr_hapdip_hap_panels/{scenario}/hap_panel_{mod_n}_{n_anc}_{{ta}}_{length}_Ne_{Ne}_{seed}.pos_ac.npz",
            mod_n=2,
            n_anc=2,
            Ne=10000,
            length=50,
    ...
    output:
        corr_SASB=config["tmpdir"]
        + "corr_seg_sites/test_corr_hapdip/autosomes.paired_seg_sites.hap0.ta{ta}.seed{seed,\d+}.monte_carlo_L{L,\d+}.N{N,\d+}.npz",
    run:
        cur_corr = CorrSegSitesRealDataHaploid()
        for f in tqdm(input.seg_sites_haploid):
            cur_corr._load_data(f, miss_to_nan=False)
...
rule test2:
    input:
        expand(
            config["tmpdir"]
            + "corr_seg_sites/test_corr_hapdip/autosomes.paired_seg_sites.hap0.ta{ta}.seed{seed}.monte_carlo_L{L}.N{N}.npz",
            L=1000,
            N=[200],
            ta=[100, 500, 1000],
            seed=[1],
...
# TEST 1
rule sim_mod_haploid_anc_haploid:
    output:
        hap_panel=config["tmpdir"]
        + "hap_copying/test_corr_haphap_hap_panels/{scenario}/hap_panel_{mod_n,\d+}_{n_anc}_{ta,\d+}_{length,\d+}_Ne_{Ne,\d+}_{seed,\d+}.npz",
    wildcard_constraints:
        scenario="(SerialConstant)",
        mod_n="1",
        n_anc="1",
