...
rule sim_demography_single_ancients:
    ...
    output:
        ...
        + "hap_copying/hap_panels/{scenario}/hap_panel_{mod_n,\d+}_{n_anc}_{ta,\d+}_{length,\d+}_Ne_{Ne,\d+}_{seed,\d+}.treeseq.gz",
        hap_panel=config["tmpdir"]
        + "hap_copying/hap_panels/{scenario}/hap_panel_{mod_n,\d+}_{n_anc}_{ta,\d+}_{length,\d+}_Ne_{Ne,\d+}_{seed,\d+}.npz",
    ...
    run:
        ...
        if scenario == "SerialConstant":
            ...
            cur_sim._add_samples(
                mod_pop=1, anc_pop=1, n_mod=mod_n, n_anc=[n_anc], t_anc=[ta]
            )
...
rule sim_demography_multi_ancients:
    ...
    output:
        treeseq=config["tmpdir"]
        + "full_sim_all/{scenario}/generations_{ta,\d+}_{interval,\d+}/serial_coal_{mod_n, \d+}_{n_anc, \d+}_{length, \d+}_Ne{Ne,\d+}_{seed,\d+}.treeseq.gz",
    wildcard_constraints:
    ...
    run:
        ...
        if scenario == "SerialConstant":
            ...
            cur_sim = SerialTennessenModel()
            cur_sim._add_samples(
                mod_pop=1, anc_pop=1, n_mod=mod_n, n_anc=n_anc, t_anc=t_anc
            )
            ...
            cur_sim = SerialMigration(
                Ne=Ne, mod_n=mod_n, t_anc=t_anc, n_anc=n_anc, mig_rate=1e-1
            )
        elif scenario == "SerialMigration2":
            cur_sim = SerialMigration(
                Ne=Ne, mod_n=mod_n, t_anc=t_anc, n_anc=n_anc, mig_rate=1e-2
            )
        elif scenario == "SerialMigration3":
            cur_sim = SerialMigration(
                Ne=Ne, mod_n=mod_n, t_anc=t_anc, n_anc=n_anc, mig_rate=1e-3
            )
        elif scenario == "SerialMigration4":
            cur_sim = SerialMigration(
                Ne=Ne, mod_n=mod_n, t_anc=t_anc, n_anc=n_anc, mig_rate=1e-4
            )
        else:
            raise ValueError("Improper value input for this simulation!")
        # Conducting the actual simulation ...
        tree_seq = cur_sim._simulate(
            mutation_rate=mut_rate,
            recombination_rate=rec_rate,
            length=length,
            random_seed=seed,
...
rule create_hap_panel_all:
    ...
    output:
        hap_panel=config["tmpdir"]
        + "full_sim_all/{scenario}/generations_{ta,\d+}_{interval,\d+}/hap_panel_{mod_n, \d+}_{n_anc, \d+}_{length, \d+}_Ne{Ne,\d+}_{seed,\d+}.panel.npz",
    run:
...
rule infer_scale_serial_all_ascertained:
    ...
    wildcard_constraints:
        scenario="(SerialConstant|SerialBottleneck|SerialBottleneckLate|TennessenEuropean|SerialBottleneckInstant[0-9]*|SerialMigration[0-9]*|IBDNeUK10K|SimpleGrowth[0-9]*)",
    output:
        mle_hap_est=config["tmpdir"]
        + "hap_copying/mle_results_all/{scenario}/generations_{ta}_{interval}/mle_scale_{mod_n, \d+}_{n_anc, \d+}_{length,\d+}_Ne{Ne,\d+}_{seed,\d+}.asc_{asc, \d+}.ta_{ta_samp, \d+}.scale.npz",
    run:
        # loading in the data
        ...
        mod_asc_panel, asc_pos, asc_idx = ascertain_variants(
            modern_hap_panel, pos, maf=np.int32(wildcards.asc) / 100.0
        )
...
rule calc_infer_scales_asc_all_figures:
    ...
    input:
        expand(
            config["tmpdir"]
            + "hap_copying/mle_results_all/{scenario}/generations_{ta}_{interval}/mle_scale_{mod_n}_{n_anc}_{length}_Ne{Ne}_{seed}.asc_{asc}.ta_{ta_samp}.scale.npz",
            ...
            seed=np.arange(1, 5),
            asc=[1, 5],
            ta_samp=np.arange(20, 501, 20),
        ),
        expand(
            config["tmpdir"]
            + "hap_copying/mle_results_all/{scenario}/generations_{ta}_{interval}/mle_scale_{mod_n}_{n_anc}_{length}_Ne{Ne}_{seed}.asc_{asc}.ta_{ta_samp}.scale.npz",
            scenario=["IBDNeUK10K", "TennessenEuropean"],
            ta=1000,
            interval=10,
            mod_n=100,
            n_anc=1,
        ...
        expand(
            ...
            + "hap_copying/mle_results_all/{scenario}/generations_{ta}_{interval}/mle_scale_{mod_n}_{n_anc}_{length}_Ne{Ne}_{seed}.asc_{asc}.ta_{ta_samp}.scale.npz",
            scenario=[
                "SerialBottleneckInstant7",
                "SerialBottleneckInstant8",
                "SerialBottleneckInstant9",
            ],
            ta=500,
            interval=10,
            mod_n=100,
...
rule concatenate_hap_copying_results:
    ...
    run:
        ...
        for f in tqdm(input.files):
            ...
            se_marginal = 1.0 / np.sqrt(-logll_deriv(scale))
            cur_row = [
                scenario,
                N,
                scale,
...
rule create_hap_panel_1kg_ceu_real_chrom:
    ...
    output:
        hap_panel=config["tmpdir"]
        + "full_sim_all/{scenario}/ceu_sim_chrX_{genmap}/serial_coal_{mod_n, \d+}_Ne{Ne,\d+}_seed_{seed,\d+}.panel.npz",
    wildcard_constraints:
    ...
    run:
        ...
        ts = cur_sim._simulate(
            mutation_rate=mut_rate, recombination_map=recmap, random_seed=seed
        )
        geno = ts.genotype_matrix().T
        phys_pos = np.array([v.position for v in ts.variants()])
...
rule infer_scale_serial_ascertained_ceu_sims:
    ...
    output:
        mle_hap_est=config["tmpdir"]
        + "hap_copying/mle_results_all/{scenario}/ceu_sim_chrX_{genmap}/mle_scale_{mod_n, \d+}_Ne{Ne,\d+}_{seed, \d+}.asc_{asc, \d+}.ta_{ta_samp, \d+}.scale.npz",
    run:
        # loading in the data
        cur_data = np.load(input.hap_panel)
        hap_panel_test = cur_data["haps"]
        ...
        mle_params = cur_hmm._infer_params(
            anc_asc_hap, x0=[start_jump, start_eps], bounds=bounds
        )
        cur_params = np.array([np.nan, np.nan])
        se_params = np.array([np.nan, np.nan])
        if mle_params["success"]:
...
times_kya_df = pd.read_csv(
    "data/hap_copying/chrX_male_analysis/mle_est_real_1kg/ceu_kya_ages.csv"
)
...
rule ceu_infer_scale_real_chrom:
    input:
        expand(
            config["tmpdir"]
            + "hap_copying/mle_results_all/{scenario}/ceu_sim_chrX_deCODE/mle_scale_{mod_n}_Ne{Ne}_{seed}.asc_{asc}.ta_{ta_samp}.scale.npz",
            scenario=[
                "SerialConstant",
                "TennessenEuropean",
                "IBDNeUK10K",
                "TennessenEuropeanXchr",
                "IBDNeUK10KXchr",
            ],
            mod_n=49,
            Ne=10000,
